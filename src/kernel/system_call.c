		...
		default:
			...
			else if(!strcmp(file_descriptor->file_name, cpu_file_name)) // Control the CPU.
			{
				...
				case CPU_COMMAND_SHUTDOWN:
					...
					printf_shell(shell, "fadt->header.revision = %#04.2x\n", fadt->header.revision);
					printf_shell(shell, "fadt->header.checksum = %#04.2x\n", fadt->header.checksum);
					printf_shell(shell, "fadt->header.oem_id = %.*s\n", _countof(fadt->header.oem_id), fadt->header.oem_id);
					printf_shell(shell, "fadt->header.oem_table_id = %.*s\n", _countof(fadt->header.oem_table_id), fadt->header.oem_table_id);
					printf_shell(shell, "fadt->header.oem_revision = %#010.8x\n", fadt->header.oem_revision);
					printf_shell(shell, "fadt->header.creater_id = %#010.8x\n", fadt->header.creater_id);
					printf_shell(shell, "fadt->header.creater_revision = %#010.8x\n", fadt->header.creater_revision);
					...
					printf_shell(shell, "fadt->smi_cmd = %#010.8x\n", fadt->smi_cmd);
					printf_shell(shell, "fadt->acpi_enable = %#04.2x\n", fadt->acpi_enable);
					printf_shell(shell, "fadt->acpi_disable = %#04.2x\n", fadt->acpi_disable);
					printf_shell(shell, "fadt->s4_bios_req = %#04.2x\n", fadt->s4_bios_req);
					printf_shell(shell, "fadt->pstate_cnt = %#04.2x\n", fadt->pstate_cnt);
					printf_shell(shell, "fadt->pm1a_evt_blk = %#010.8x\n", fadt->pm1a_evt_blk);
					printf_shell(shell, "fadt->pm1b_evt_blk = %#010.8x\n", fadt->pm1b_evt_blk);
					printf_shell(shell, "fadt->pm1a_cnt_blk = %#010.8x\n", fadt->pm1a_cnt_blk);
					printf_shell(shell, "fadt->pm1b_cnt_blk = %#010.8x\n", fadt->pm1b_cnt_blk);
					printf_shell(shell, "fadt->pm2_cnt_blk = %#010.8x\n", fadt->pm2_cnt_blk);
					printf_shell(shell, "fadt->pm_tmr_blk = %#010.8x\n", fadt->pm_tmr_blk);
					printf_shell(shell, "fadt->gpe0_blk = %#010.8x\n", fadt->gpe0_blk);
					printf_shell(shell, "fadt->gpe1_blk = %#010.8x\n", fadt->gpe1_blk);
					printf_shell(shell, "fadt->pm1_evt_len = %#04.2x\n", fadt->pm1_evt_len);
					printf_shell(shell, "fadt->pm1_cnt_len = %#04.2x\n", fadt->pm1_cnt_len);
					printf_shell(shell, "fadt->pm2_cnt_len = %#04.2x\n", fadt->pm2_cnt_len);
					printf_shell(shell, "fadt->pm_tmr_len = %#04.2x\n", fadt->pm_tmr_len);
					printf_shell(shell, "fadt->gpe0_blk_len = %#04.2x\n", fadt->gpe0_blk_len);
					printf_shell(shell, "fadt->gpe1_blk_len = %#04.2x\n", fadt->gpe1_blk_len);
					printf_shell(shell, "fadt->gpe1_base = %#04.2x\n", fadt->gpe1_base);
					printf_shell(shell, "fadt->cst_cnt = %#04.2x\n", fadt->cst_cnt);
					printf_shell(shell, "fadt->p_lvl2_lat = %#06.4x\n", fadt->p_lvl2_lat);
					printf_shell(shell, "fadt->p_lvl3_lat = %#06.4x\n", fadt->p_lvl3_lat);
					printf_shell(shell, "fadt->flush_size = %#06.4x\n", fadt->flush_size);
					printf_shell(shell, "fadt->flush_stride = %#06.4x\n", fadt->flush_stride);
					printf_shell(shell, "fadt->duty_offset = %#04.2x\n", fadt->duty_offset);
					printf_shell(shell, "fadt->duty_width = %#04.2x\n", fadt->duty_width);
					printf_shell(shell, "fadt->day_alrm = %#04.2x\n", fadt->day_alrm);
					printf_shell(shell, "fadt->mon_alarm = %#04.2x\n", fadt->mon_alrm);
					printf_shell(shell, "fadt->century = %#04.2x\n", fadt->century);
